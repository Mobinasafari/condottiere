...
class player{
	...
	public :
		...
		void score_calculation( string season , bool tablzan_card)
		{
			int zarib = 1 ;
			if ( tablzan_card == true)
			{
				zarib = 2 ;
			}
...
class control{
	...
	void play_card( int playerr , int number_of_players )
	{
		...
		if ( playerArry[playerr].get_pass() == 1 )
		{
			...
			while ( test == false)
			{
				...
				if ( card_name == "pass" ||card_name == "1" || card_name == "2"  || card_name == "3" || card_name == "4" || card_name == "5" || card_name == "6" || card_name == "10"|| card_name == "matarsak"|| card_name == "tablzan"|| card_name == "bahar"|| card_name == "zemestan")          //khataaaaaaaaaaaaaaaa
				{
					test = true ;
				}
			}
	...
	void Determine_the_winner( int number_of_players )
	{
		for ( int i = 0 ; i < number_of_players ;  i++)
		{
			playerArry[i].score_calculation(season , tablzan_card);
		}
		starter = 1 ;
		for ( int i = 0 ; i < number_of_players ; i++)
		{
