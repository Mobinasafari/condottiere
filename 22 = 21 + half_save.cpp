...
class player{
	...
	public :
		...
		void print_all_hand_cards( int andis , bool saving )
		{
			if ( saving == false)
			{
...
class control{
	...
	void save(int number_of_players)
	{
		saving = true ;
		ofstream my_file1;
		my_file1.open("game.txt");
		my_file1 << "1   " << number_of_players << endl ;
			
		for ( int i = 0 ; i < number_of_players ; i++)
		{
			my_file1 << "p" << i+1 << "  " << playerArry[i].get_name() << "  " << playerArry[i].get_age() << endl ;
