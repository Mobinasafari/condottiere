...
class player{
	...
	public :
		...
		void score_calculation( string season )   //be tartib olaviat neveshte shode zemestan .. tablzan .. bahar
		{
			cout << endl << "score_calculation is starting   " << season << endl ;
			score = 0 ;
			bool effect = false ;
			...
			if ( season == "bahar" && effect == false )
			{
				for ( int i = 0 ; i < number_of_played_card ; i++ ) ;
				effect = true ;
			}
			if ( season == "nul" && effect == false )
			{
				for ( int i = 0 ; i < number_of_played_card ; i++ )
				{	
...
class control{
	...
	void play_card( int playerr , int number_of_players )
	{
		...
		if ( playerArry[playerr].get_pass() == 1 )
		{
			...
			while ( test == false)
			{
				...
				if ( card_name == "pass" ||card_name == "1" || card_name == "2"  || card_name == "3" || card_name == "4" || card_name == "5" || card_name == "6" || card_name == "10"|| card_name == "matarsak"|| card_name == "tablzan"|| card_name == "bahar"|| card_name == "zemestan" || card_name == "shahdokht" )          //khataaaaaaaaaaaaaaaa
				{
					test = true ;
				}
			}
		...
		if ( card_name == "pass")
		{
			...
			{
				playerArry[playerr].set_tablzan_card();
			}
			if ( card_name == "bahar" )
			{
				season = "bahar" ;
	...
	void Determine_the_winner( int number_of_players )
	{
		for ( int i = 0 ; i < number_of_players ;  i++)
		{
			playerArry[i].score_calculation(season);
		...
		cout << endl << "the winner is player " << starter ;
