...
class Rakhsh : public perpule_card {
	
	public :
		Rakhsh( string input_name , int input_number_of_card , int input_value) : perpule_card(input_name , input_number_of_card , input_value)
		{
			
		}
		virtual void print(int &score)
		{
			cout << "Rakhsh_sefid" << "  " << score ;
		}
		
};
class Fok_sefid : public perpule_card {
	
	public :
		Fok_sefid( string input_name , int input_number_of_card , int input_value) : perpule_card(input_name , input_number_of_card , input_value)
		{
			
		}
		virtual void print(int &score)
		{
			cout << "Fok_sefid" << "  " << score ;
		}
...
class player{
	...
	public :
		...
		void set_number_of_play_cards( int input_number_of_play_cards)
		{
			number_of_play_card = input_number_of_play_cards ;
		}
		...
		void set_tablzan_card( int tablzan)
		{
			tablzan_card = 	tablzan ;
		}
		...
		int get_number_of_play_card()
		{
			return number_of_play_card ;
		}
		int get_pass()
		{
			return pass;
		...
		void print_played_card(int andis , bool saving)
		{
			if ( saving == false)
			{
				...
				for ( int i = 0 ; i < number_of_play_card ; i++)
				{
			...
			else 
			{
				ofstream my_file1;
				my_file1.open("game.txt" , ios::app);
				my_file1 << endl << "number_of_play_cards_player " << andis+1 << "  " << number_of_play_card << endl;
				for ( int i = 0 ; i < number_of_play_card ; i++)
				{
					my_file1 << Arry_of_hand_played_card[i] -> get_name() << "  ";
				}
				my_file1 << number_of_played_card << "  ";
			}
		...
		{
			Arry_of_hand_played_card[number_of_play_card] = Arry_of_hand_card[andis] ;
		}
		...
		void matarsak( int andis )
		{
			...
			for ( int i = 0 ; i < number_of_play_card ; i++)
			{
				if ( Arry_of_hand_played_card[i] -> get_name() == "matarsak" )
				{
					find = true ;
				}
			}
			if ( find == true )
			{
				cout << endl << "player " << andis + 1 << " choise your card that you want to return : " << endl ;
				string return_card ;
				cin >> return_card ;
				...
				for ( int i = 0 ; i < number_of_play_card ; i++)
				{
					if ( Arry_of_hand_played_card[i] -> get_name() == return_card  )
					{
						...
						for ( int j = i ; j < number_of_play_card  ; j++)
						{
							Arry_of_hand_played_card[j] = Arry_of_hand_played_card[j+1] ;	
						}
						number_of_played_card = number_of_played_card - 1 ;
						number_of_play_card = number_of_play_card - 1 ;
						break;
					}
				}
			}
		}
		void score_calculation( string season , int biggest_number , int Lucky_number)   //be tartib olaviat neveshte shode zemestan .. tablzan .. bahar
		{
			...
			if ( season == "zemestan" && effect == false ) 
			{
				...
				{	
					...
					{
						score = score + (Arry_of_hand_played_card[i] -> get_value() / 2 );
					}
			...
			if ( tablzan_card > 0  && effect == false )
			{
				int num ;
				float zarib = 1 ;
				for ( int i = 0 ; i < tablzan_card ; i++)
				{
					zarib = zarib * 1.5 ;
				}
				for ( int i = 0 ; i < number_of_play_card ; i++ )
		...
		int bahar()
		{
			...
			for( int i = 0 ; i < number_of_play_card ; i++)
			{
				if ( Arry_of_hand_played_card[i] -> get_value() > num   )
				{
					num = Arry_of_hand_played_card[i] -> get_value() ;
				}
			}
		...
		void save()
		{
			...
			my_file1.open("game.txt" , ios::app);
			my_file1 << endl ;
			my_file1 << name << "  " << age << "  " << pass << "  " << score << "  " << number_of_ostan_fath_shode << "  " << tablzan_card << "  " << bahar_card << "  " << shirzan_card << "  " << red_card << "  " ;
		}
...
class ostan{
	...
	public  :
		...
		void set_counter_save ( int num , int andis)
		{
			counter[andis] = num ;
		}
		...
		void save()
		{
			ofstream my_file1;
			my_file1.open("game.txt" , ios::app);
			my_file1 << name << "  " << status << "  " ;
			for ( int i = 0 ; i < 6 ; i++)
			{
				my_file1 << counter[i] << "  ";
			}
			my_file1 << player_name << "  ";
		}
...
class control{
	...
	int reading_file()
	{
		...
		for ( int i = 0 ; i <  10 ; i++)                                        //khataaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa
		{
			...
			if ( input == "player" )
			{
				...
				{
					my_file >> input ;
					playerArry[i].set_name( input );
					
					my_file >> num ;
					playerArry[i].set_age( num );
					
					my_file >> num ;
					playerArry[i].set_pass( num );
					
					my_file >> num ;
					playerArry[i].set_score( num );
					
					my_file >> num ;
					playerArry[i].set_number_of_ostan_fath_shode( num );   
					
					my_file >> num ;
					playerArry[i].set_tablzan_card( num );
					
					my_file >> num ;
					playerArry[i].set_bahar_card( num );
					
					my_file >> num ;
					playerArry[i].set_shirzan_card( num );
					
					my_file >> num ;
					playerArry[i].set_red_card( num );
					
				
					playerArry[i].set_number_of_cards(10) ;
			...
			if ( input == "number")
			{
				...
				for ( int i = 0 ; i < num ; i++)
				{
					...
					{
						playerArry[andis-1].set_arry( card_Arry[18] , i );
					}
					if ( input == "fok_sefid")
					{
						playerArry[andis-1].set_arry( card_Arry[19] , i );
					}
				}
			}
			if ( input == "number_of_play_cards_player")
			{
				my_file >> num ;
				int andis = num ;
				my_file >> num ;
				
				playerArry[andis-1].set_number_of_play_cards(num);
				...
				for ( int i = 0 ; i < num ; i++)
				{
					...
					if ( input == "rakhsh_sefid")
					{
						playerArry[andis-1].add_played_card_for_saving( card_Arry[18] , i );
					}
					if ( input == "fok_sefid")
					{
						playerArry[andis-1].add_played_card_for_saving( card_Arry[19] , i );
					}
				}
				my_file >> num ;
			...
			if ( input == "ostan")
			{
				for ( int i = 0 ; i < 15 ; i++)
				{
					my_file >> input ;
					my_file >> num ;
					vector_of_ostans[i].set_status(num);
					for ( int j = 0; j < 6 ; j++)
					{
						my_file >> num ;
						vector_of_ostans[i].set_counter_save(num , j) ;
					}
					my_file >> input ;
					if ( input != "nul" )
					{
						vector_of_ostans[i].set_player_name(input) ;
					}
				}
			}
		}
	...
	void card_initializer( int number_of_player)
	{
		...
		Rakhsh rakhsh_sefid("rakhsh_sefid" , 2 , 0);
		card_Arry[18] = &rakhsh_sefid ;
		
		Fok_sefid fok_sefid("fok_sefid" , 10 , 0);
		card_Arry[19] = &fok_sefid ;
	...
	void ostan_initializer()
	{
        ...
        vector_of_ostans[2].set_name("LIA");
        ...
        vector_of_ostans[3].set_name_of_neighbors("NEW");
        ...
        vector_of_ostans[6].set_name_of_neighbors("NEW");
    	...
    	vector_of_ostans[12].set_name("ENNA");
        ...
        vector_of_ostans[14].set_name("NEW");
        vector_of_ostans[14].set_jungle(0);
		vector_of_ostans[14].set_ghale(0);
		vector_of_ostans[14].set_koohestan(0);
        vector_of_ostans[14].set_name_of_neighbors("ATELA");
        vector_of_ostans[14].set_name_of_neighbors("DIMASE");
	...
	void play_card( int playerr , int number_of_players )
	{
		...
		if ( card_name == "pass")
		{
			...
			{
				playerArry[playerr].set_tablzan_card(playerArry[playerr].get_tablzan_card() + 1 );
			}
			if ( card_name == "bahar" )
			{
				season = "bahar" ;
			...
			if ( card_name == "rakhsh_sefid")
			{
				Rakhsh_sefid(playerr , number_of_players);
			}
			if ( card_name == "fok_sefid")
			{
				for ( int i = 0 ; i < number_of_players ; i++)
				{
					playerArry[i].set_number_of_play_cards(0);
				}
			}
	...
	void Rakhsh_sefid( int playerrr , int number_of_players)
	{
		starter = playerrr + 1 ; //choon playerrr andis ast va starter shomare bazikon
		
		cout << endl << "the winner whit playing rakhsh_sefid is player " << starter << endl ;
	...
	void recavery(int number_of_players , int stutus)
	{
		...
		if ( stutus == 2 )
		{
			...
			for ( int i = 0 ; i < number_of_players ; i++)
			{
				...
				playerArry[i].set_shirzan_card(0);
				bool bahar = false ;
				playerArry[i].set_bahar_card(bahar);
				playerArry[i].set_tablzan_card(0);
			}
		}	
	}
	...
	void Determine_the_winner( int number_of_players )
	{
		...
		{
			playerArry[i].score_calculation(season , biggest_number , Lucky_number);
	...
	int get_gang() //Emtiaziiiiiiii  //jaygozin cin >> gang 
	{
		...
		while ( number == false )
		{
			cout << endl << "Player " << starter <<  " Enter lucky number between 9 and 100" << endl  ;
			cin >> Lucky_number ;
			if ( Lucky_number > 9 && Lucky_number < 100)
			{
				number = true ;
			}
		}
